...
std::string ProcessParser::getVmSize(std::string pid){
    ...
    std::string result;
    std::ifstream my_ifs;
    Util::getStream(Path::basePath() + pid + Path::statusPath(), my_ifs);
    std::string line;
    //read line by line
    while(std::getline(my_ifs, line)){
        if(line.substr(0,6) == "VmData"){
            //found the VmData line
            std::istringstream my_iss(line);
            my_iss >> result;
            my_iss >> result;
            //convert kB to GB
            float temp = stof(result)/float(1024);
...
string ProcessParser::getCpuPercent(string pid)
{
    ...
    float result;
    ifstream my_ifs;
    Util::getStream((Path::basePath()+ pid + "/" + Path::statPath()), my_ifs);
    getline(my_ifs, line);
    string str = line;
    istringstream buf(str);
    istream_iterator<string> begin(buf), end;
    vector<string> values(begin, end); // done!
